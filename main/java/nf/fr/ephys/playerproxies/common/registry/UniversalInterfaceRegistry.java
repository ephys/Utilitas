...
public class UniversalInterfaceRegistry {
	private static TreeMap<Class<?>, Class<? extends UniversalInterface>> map = new TreeMap<>(
	...
	public static void addInterface(Class<? extends UniversalInterface> clazzInterface, Class<?> clazzTarget) {
		if (map.containsKey(clazzTarget)) {
			PlayerProxies.getLogger().error(clazzTarget + " already has a class handler. Ignoring " + clazzInterface);
			return;
		}
